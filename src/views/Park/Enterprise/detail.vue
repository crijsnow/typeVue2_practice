          ...
          >
            ...
            >
              <template #default="scope">
                <el-button type="text" @click="down(scope.row)">合同下载</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
<script>
import { getdetial, download } from '@/apis/enterprise'
export default {
  data() {
    return {
      form: {},
  ...
  methods: {
    async getDetail() {
      const res = await getdetial(this.$route.query.id)
      res.data.rent.forEach(obj => {
        const extNamea = obj.contractUrl.split('.')
        const extName = extNamea[extNamea.length - 1]
        if (extName === 'doc') {
    ...
    async down(row) {
    //   console.log(id)
      const res = await download(row.contractId)
      console.log(res)
